...
**Actions.** A slot exists only after start succeeds. Stored live phase is `Windup`, `Active`, or `Recovery`; otherwise terminal. `ActionDenialReason::TransitionLocked` absorbs `InputGateReason` without rewriting the ADR-0042 remaining-tick lock. `InteractionSession` stays distinct from Action. Phase **9A/9B** abilities use this table (`ActionKind::Ability` + `AbilityDefinition`). Activation (`Independent` / `SelectedEntity`) is distinct from delivery (`SelfTarget` / `ForwardQuery` / `SelectedEntity`). Instant effects are `AbilityEffect` (`Damage`, `Dash`); `TempEffect` stays duration-based. 7.2 `Strike` remains workload-only. Phase **9C** is the client→server activation command; combat execution stays in `World::request_ability`. Phase **9D** maps ability execution → Attack oneshot, damage → Hurt oneshot, and `Health <= 0` → persistent Dead presentation (ADR-0063). Dash uses this same action/cooldown/grant path and a semantic Dash oneshot; it does not add a parallel state machine.
...
It does **not** yet own: moving platforms, ropes, ladders, slopes, ice/conveyor surfaces, wall-jumps, crouch, CCD-as-engine, or content JSON tuning. Dash v1 is the first ability-driven extension: FOOTNOTE resolves its authoritative horizontal motion against normal world bounds, Solid collision, support and edges; ability/grant/cooldown ownership remains outside the controller.
