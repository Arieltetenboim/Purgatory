...
The existing `Graphic/` directory stays in place. Phase 5.0B loads **only** `Graphic/LOGO.png` for the Connection Frontend (temporary filesystem path). The game client compile-embeds one Headwear Side proof PNG for the Crown attachment; that is not a Graphic/ scan or asset pipeline. Do not modify files in `Graphic/` as part of networking work.
