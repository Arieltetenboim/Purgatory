...
Startup: `ClientScreen::Connection`, `ConnectionState::Disconnected`. In the default diagnostic build the network thread stays idle until an explicit DEV Connect command; R2 foreground navigation never connects. The existing no-default-features shipping auto-connect path is retained (there are no egui controls in that build). There is no reconnect loop.
...
The temporary Connection Frontend is `ConnectionFrontend::paint` (egui, existing window, under `dev-diagnostics`). Frontend Navigation Runtime R2 adds the always-compiled `frontend_runtime.rs`: `ClientLifecycle` owns only Connection/Game, `FrontendRuntime` owns Intro/Login/ChannelSelect/CharacterSelect and local selections, and `FrontendScene` owns camera motion. Intro holds for **1.25 seconds** of supplied frame dt, then requests Login. The mappings are Intro → Intro, Login → Login, ChannelSelect → Channel, CharacterSelect → Character. A request hides foreground controls and locks navigation; the current stage stays unchanged until `FrontendScene::is_at` reports arrival, then target controls appear. No second completion timer, wall-clock navigation reads, retargeting, or action queue is used. F5–F8 use the same request/lock path; revisiting Intro restarts its delay after camera arrival.

Login retains its username form and validation, with the former ENTER PURGATORY connection action temporarily replaced by local CONTINUE. Two local channel slots lead to CharacterSelect; three character slots only update a local selection. Back moves CharacterSelect → ChannelSelect → Login and cannot return Login to Intro. Zero-based placeholder selections persist across these paths for the process lifetime only. These actions neither send network commands nor mutate world addresses; there is no Enter World. The overlay transports the local action separately from DEV network commands. `~` and the existing DEV network controls remain available; gameplay camera and lifecycle semantics are unchanged. The runtime and scene compile without diagnostic egui, while the temporary controls remain diagnostic-only.

`Graphic/frontend/LOGO.png` is loaded **once** at frontend init into an `egui::TextureHandle` via a centralized path helper (`apps/client/src/assets.rs`). This load path is temporary. Missing/decode failure logs one warning and falls back to the text title `PURGATORY`. The game client compile-embeds the four Animation Lab Headwear Side proof cells, registers their sprite metadata in the client `AssetRuntime`, and resolves Crown attachment visuals through the shared visual-key path (no filesystem loader). Frontend Scene Runtime R1 loads `Graphic/frontend/frontend_scene_guide.png` through `ClientAssetLoader` into `AssetRuntime` before renderer initialization. The always-compiled `frontend_scene.rs` owns fixed 2880x5120 scene framing, a 2560x1440 crop at X=160, and deterministic one-second smoothstep transitions at Y=0/1227/2453/3680. A single `UiTexturedRect` fills the constrained 16:9 framebuffer viewport independently of UI Scale. DEV F5/F6/F7/F8 request Intro/Login/ChannelSelect/CharacterSelect through FrontendRuntime while on Connection; lifecycle and gameplay camera ownership are unchanged. Legacy egui backgrounds and the opaque splash are removed; foreground controls remain. Frontend artwork lives only in `Graphic/frontend/`.
...
→ Connection: FrontendRuntime → FrontendScene + ConnectionFrontend::paint (stage-specific temporary controls); Game: parallax → AABBs (local player uses the finalized presentation pose) → optional gizmos
