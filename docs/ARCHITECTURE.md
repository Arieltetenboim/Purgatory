...
Startup: `ClientScreen::Connection`, `ConnectionState::Disconnected`. In both default and no-default-features builds the network thread stays idle during local R3 frontend navigation. Only the existing diagnostic overlay can issue an explicit DEV Connect command; there is no shipping auto-connect. There is no reconnect loop.
...
Login accepts click focus, append-only printable username input, Backspace, and valid Enter/CONTINUE. The single-line append primitive takes its acceptance policy from the caller; this temporary Login caller uses the existing `DevLogin` character alphabet, maximum length, and full submission validation. It establishes no future character-name rules. Two local placeholder channels navigate to CharacterSelect without network traffic or world-address changes. Both diagnostic and shipping builds remain in local frontend navigation; the former shipping auto-connect bypass is removed. Existing DEV network controls remain available in the overlay, and lifecycle/Game entry semantics are unchanged.

Character Creation V1 (R4) remains a Character-area sub-mode, never a frontend stage: `FrontendStage::CharacterSelect` and `FrontendSceneStop::Character` remain unchanged throughout Browsing → Creating → Browsing. The area owns exactly three process-local proof slots, initially Occupied/Empty/Occupied, with owned display names and no authoritative IDs. Selecting an Empty slot exposes CREATE NEW CHARACTER. Only that selected Empty slot can enter `CharacterAreaMode::Creating { slot }`; `CharacterArea` owns an optional `CharacterCreationDraft { slot, name }`, initially empty. Other slots and channel navigation are locked during creation.

Character names use their own `validate_character_name` policy: **3–12 ASCII alphanumeric characters**, preserving entered case. Spaces, underscores, dots, punctuation, symbols and Unicode are rejected. The existing single-line input primitive supports click focus, Backspace, and filtered/capped entry. CREATE is disabled until valid; focused Enter submits only a valid active draft. Uniqueness is deferred: future checks will be server-authoritative and case-insensitive. R4 performs no uniqueness check and makes no name-availability claim.

Valid CREATE changes the current Empty slot to Occupied locally, stores the entered display name, selects it as the local playable selection, returns to Browsing and clears the draft without requesting camera motion. CANCEL and Back/Escape clear the draft and keep the slot Empty at Character; re-entry starts fresh. Only Back from Browsing navigates to ChannelSelect. Leaving CharacterSelect via DEV navigation also clears draft/mode. Occupied slots have no creation controls; selection remains local. This proof has no server request, persistence, filesystem/account save, character ID generation, deletion, replacement or Enter World.
