...
The Character area owns exactly three deterministic local proof slots (Occupied, Empty, Occupied), with no authoritative IDs or persistence. Area selection is distinct from playable selection: an Empty slot cannot resolve to a selected character. Selecting it exposes CREATE NEW CHARACTER; that action enters `CharacterAreaMode::Creating { slot }` inside the same slot region. NAME, APPEARANCE, CLOTHING, and ATTRIBUTES are placeholders only. Other slots are non-interactive during creation. CANCEL or Back/Escape first returns to Browsing without moving the camera or changing stage; Back from Browsing requests ChannelSelect. There is no CharacterCreate stage, creation submission, or Enter World. Connection pointer and keyboard events return before game UI/gameplay handling; an open diagnostic overlay takes priority over frontend input.
