...
Startup: `ClientScreen::Connection`, `ConnectionState::Disconnected`. Login CONTINUE validates DevLogin and starts a connection on the existing network thread in both builds. Duplicate Continue is disabled while connecting/handshaking. There is no auto-connect or reconnect loop.

R5B accepts `FrontendSessionReady` during Handshaking, yielding `ConnectionState::Connected` while keeping `ClientScreen::Connection`. Generic `Connected` never changes screen. Only explicit `GameplayReady` from authoritative Welcome can enter Game; the production R5B server never sends Welcome. QUIC transport-up is not enough. Stale events from older attempts are ignored. Disconnect invalidates the active attempt **immediately** on the client (before the runtime ack). A delayed Welcome after Disconnect cannot enter Game.
...
Production Frontend R3 uses the always-compiled `frontend_ui.rs` for Login, Channel, and Character controls. It submits `UiRect`, `UiTexturedRect`, and `TextBlock` compositions to the existing renderer and reuses `ScreenRect` hit testing and production press/release conventions. Broad frontend buttons use flat rectangles because the existing 18px atlas art intentionally tiles rather than upscales; gameplay button assets and renderer behavior are unchanged. Drawing and hit regions share a 1280x720 reference layout fitted to the constrained framebuffer viewport; font sizes compensate for the renderer's UI scale so the frontend fits that viewport at every display scale. Diagnostic egui retains the debug overlay and existing gameplay HUD, but owns no frontend controls.

Ownership stays separate: `ClientLifecycle` owns Connection/Game; `FrontendRuntime` owns Intro/Login/ChannelSelect/CharacterSelect and local Character-area state; `FrontendScene` owns camera motion. Intro holds for **1.25 seconds** of supplied frame dt, then requests Login. The mappings remain Intro -> Intro, Login -> Login, ChannelSelect -> Channel, CharacterSelect -> Character. During a request, foreground controls are hidden and input is locked until camera arrival. F5-F8 use the same request/lock path. Leaving CharacterSelect clears creation mode and draft; returning starts in Browsing.

Login accepts click focus, append-only printable username input, Backspace, and valid Enter/CONTINUE. The single-line append primitive takes its acceptance policy from the caller; this temporary Login caller uses the existing `DevLogin` character alphabet, maximum length, and full submission validation. It establishes no future character-name rules. Two local placeholder channels navigate to CharacterSelect without network traffic or world-address changes. Both builds wait for the authoritative roster before advancing Login to ChannelSelect. DEV Connect uses the same pre-game session. Back from ChannelSelect disconnects before returning to Login, so editing a login cannot silently reuse another profile session.

Character Creation V1 (R4) remains a Character-area sub-mode, never a frontend stage: `FrontendStage::CharacterSelect` and `FrontendSceneStop::Character` remain unchanged throughout Browsing → Creating → Browsing. R5B replaces the proof roster with exactly three display slots projected from the authoritative ordered roster: entries 0–2 become Occupied with CharacterId and display name; missing entries are Empty. Initial and disconnected projections are all Empty. Selecting an Empty slot exposes CREATE NEW CHARACTER. Only that selected Empty slot can enter `CharacterAreaMode::Creating { slot }`; `CharacterArea` owns an optional `CharacterCreationDraft { slot, name }`, initially empty. Other slots and channel navigation are locked during creation.

Character names use their own `validate_character_name` policy: **3–12 ASCII alphanumeric characters**, preserving entered case. Spaces, underscores, dots, punctuation, symbols and Unicode are rejected. The existing single-line input primitive supports click focus, Backspace, and filtered/capped entry. CREATE is disabled until valid; focused Enter submits only a valid active draft. Local shared CharacterName validation is UX only. The persistence owner independently enforces validity, global ASCII-case-insensitive uniqueness and capacity.

Valid CREATE sends only the draft name and enters explicit Pending without changing a slot. Pending locks creation, cancellation, editing and diagnostic navigation. Success replaces the complete ordered roster, selects the newly returned CharacterId, clears the draft and returns to Browsing without camera motion. InvalidName, NameTaken, RosterFull and StorageFailure preserve the workspace/draft and allow retry. Before submission, CANCEL and Back clear the draft locally. Occupied selection stays local; there is no Enter World request. Disconnect clears roster, selection, draft and pending state, retargets Login even during camera travel, and a new attempt fetches the roster again. Lifecycle filters stale roster/create events before frontend projection.

The selected slot workspace displays a fixed base character, NAME, APPEARANCE, ATTRIBUTES, CREATE and CANCEL. Eyes, Hair, Hair Color, Eye Color, Shirt, Pants and Shoes are all visible with DEFAULT values under a LOCKED heading, and have no controls or stored draft values. Attributes are reserved but undefined, with “Customization available later”; no stats or point budgets exist. The fixed preview reuses the canonical bind-pose preparation, base-piece draw order, existing `textured_base_quad` composer and already-loaded character visual pack; no gameplay entity or second character renderer is created. Cached base quads are fitted into each occupied/creating slot by the same frontend `Layout`. `UiTexturedQuad` submits their existing corners/UVs through the shared UI texture pipeline, registry and primitive budget, without rectangle tiling; existing UI rectangle behavior stays unchanged.

Connection pointer and keyboard events return before game UI/gameplay handling; an open diagnostic overlay takes priority over frontend input.

`Graphic/frontend/LOGO.png` is canonical runtime art registered once as `frontend.logo` by `ClientAssetLoader` in the existing `AssetRuntime`, before renderer creation. The frontend UI receives its texture ID and aspect ratio, never reads files, and has no egui texture loader. Missing/decode errors follow the normal production loader startup-error path. Frontend Scene Runtime R1 loads `Graphic/frontend/frontend_scene_guide.png` through the same loader. The always-compiled `frontend_scene.rs` retains fixed 2880x5120 scene framing, a 2560x1440 crop at X=160, and deterministic one-second smoothstep transitions at Y=0/1227/2453/3680. One `UiTexturedRect` fills the constrained 16:9 framebuffer viewport independently of UI Scale. Browse/Create changes never call this scene owner. Frontend artwork lives only in `Graphic/frontend/`. Existing embedded Headwear proof resources are unchanged.
...
→ Connection: FrontendRuntime → FrontendScene + FrontendUi (production compositions and local UI intents); Game: parallax → AABBs (local player uses the finalized presentation pose) → optional gizmos
...
- **Owner:** `apps/client/src/display.rs` (`DisplaySettings` / `DisplayController`). Debug UI and the player-facing game Settings window submit the same display intent; the production UI does not own a second settings model.
...
- **UI scale** — multiplier on the OS / winit scale factor (`pixels_per_point = os_scale × ui_scale`). Independent of resolution and of render scale so a 2560×1440 window does not shrink debug/game UI. Player presets are 90% / **100%** / 110% / 125%; default `1.0` (100%).
- **Window mode:** `Windowed` or borderless fullscreen on the current monitor. Borderless fullscreen uses winit's normal monitor-sized path and preserves the selected windowed resolution for restoration. Windowed mode is not manually resizable; its size comes from the selected resolution. Exclusive fullscreen and video-mode selection are not supported.
- **Persistence:** deferred. There is no local client settings file yet. Do not store display configuration on the server.
- **Resize lifecycle:** Debug/Settings intent → `DisplayController` → `Window::request_inner_size` for windowed resolution or `Window::set_fullscreen(Borderless(current_monitor))` for fullscreen → `WindowEvent::Resized` / immediate windowed apply → `classify_framebuffer_resize` → `Renderer::resize` (swapchain; skip zero-size and unchanged) → `ensure_world_target` (recreate offscreen RT if gameplay-rect × scale size **or** world MSAA sample count changed) → clamp camera center to `WorldBounds` (world FOV is unchanged). Manual OS dragging is disabled, while unavoidable OS events such as fullscreen and DPI transitions still use the shared observe/reconfigure path. A fullscreen framebuffer observation never overwrites the saved windowed resolution. Render Scale and world MSAA changes skip the window path and only rebuild the world target.
...
Production Asset Loader A0: `apps/client/src/assets.rs::ClientAssetLoader` owns synchronous PNG filesystem loading into the existing `AssetRuntime`. Construct it with `&mut AssetRuntime` and call `load_png("frontend.scene.guide", "frontend/frontend_scene_guide.png")`. Paths are relative to the checkout's `Graphic/` directory, resolved from `CARGO_MANIFEST_DIR`, never the process working directory. The production frontend logo uses this same root and loader with key `frontend.logo`. Release packaging is not yet defined; a relocated executable still requires that build-time checkout path. Logical keys use the registry's existing first-registration-wins policy: repeat requests return the registered texture without reading or decoding again, including when the caller supplies a different source path or the key was registered from embedded bytes. Keys must be nonempty ASCII letters/digits or `.`, `_`, `-`; absolute paths and traversal components are rejected. Missing files, read failures, and PNG decode failures include the key and source path and do not register resources. Load before renderer initialization: `Renderer::new` uploads `AssetRuntime::resources()`; A0 adds no live texture upload mechanism. Tests cover generated one-pixel fixtures and canonical frontend scene/logo registration.
...
### R5B pre-game ownership (ADR-0066)

Validated DevLogin is retained by the live connection. Hello reads the roster through the existing single mutable persistence worker; it never resolves/creates a compatibility character, reserves occupancy, calls GameplayTx::enter, spawns a player or starts replication. Roster and CreateCharacter commands use the same reliable control stream. The worker runs filesystem work in `spawn_blocking`; connection tasks await owned replies. Create success is published only after persistence commits. Gameplay occupancy logic remains intact for the next entry slice.

Historical network/gameplay integration fixtures explicitly prefix their build label with `legacy-test:`. Only `cfg(test)` binaries honor this fixture and retain their legacy Enter/Welcome setup; shipping builds have no such bypass. New frontend integration tests exercise the normal pre-game path with a gameplay owner present. The readiness probe consumes FrontendSessionReady without creating a character. Gameplay load bots fail with an explicit selected-character-entry dependency until the next slice.

Existing diagnostic counters named `welcome_*` measure the first accepted session response in R5B (FrontendSessionReady); they are not evidence of gameplay entry. The historical names are retained to avoid changing the load metrics schema in this slice.
