...
## Character Creation V1 R4 — local contract and production UI

- Date: 2026-09-24
- Status: automated pass; native rendering/cancel and resulting local creation observed, with remaining manual coverage below.
- Base: required R3 commit `789c0e41e4ae6fa29b34b9db1b93305b4ebe0a54`; branch `frontend/character-creation-r4` (master did not contain R3).
- `cargo test -p purgatory-client frontend`: pass, 32 tests (all 27 R1–R3 tests retained).
- `cargo test -p purgatory-client frontend_runtime`: pass, 14 tests.
- `cargo test -p purgatory-client frontend_scene`: pass, 6 tests.
- `cargo check -p purgatory-client`: pass.
- `cargo check -p purgatory-client --no-default-features`: pass.
- `cargo clippy -p purgatory-client --all-targets --all-features -- -D warnings`: pass after fixing a collapsible-if style warning.
- `cargo test -p purgatory-client renderer::ui::tests`: pass, 4 tests.
- `cargo test -p purgatory-client renderer_repeated_text_preserves_order_and_color -- --ignored`: pass, 1 GPU framebuffer regression test for the shared composition path.
- `cargo build -p purgatory-client`: pass; final binary used for native inspection.
- `rustfmt --edition 2024 --check --config skip_children=true apps/client/src/app.rs apps/client/src/frontend_runtime.rs apps/client/src/frontend_ui.rs apps/client/src/character_presentation/debug_visual.rs apps/client/src/character_presentation/mod.rs apps/client/src/renderer/ui.rs apps/client/src/renderer/gpu.rs apps/client/src/renderer/mod.rs apps/client/src/renderer/text_gpu_tests.rs`: pass.
- `git diff --check`: pass.
- Focused coverage: ASCII alphanumeric 3/12 boundaries and 2/13 rejection; spaces/underscore/dot/punctuation/Unicode rejection; display case preservation; filtered/capped input, click focus, Backspace, Enter validation and repeat suppression; pointer CREATE validation; Empty-only creation; occupied-slot protection; fresh/cancelled/departed drafts; other slots and channel navigation locked; locked-slot clicks preserve name focus; Empty → Occupied with entered name, selection, draft clearing and unchanged Character camera; Browsing Back still reaches Channel; all seven locked DEFAULT appearance rows and undefined attributes; 14 shared base sprite pieces fitted into occupied/creating slots; constrained layouts at 800x600, 1280x720, 1920x1009, 1920x1080 and 2560x1080, with UI scale compensation.
