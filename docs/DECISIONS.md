...
- Status: accepted
- Decision: `World` owns a uniform grid per live `WorldAddress`. Cell size is configuration (`SPATIAL_CELL_SIZE_WU = 4.0` wu initially, not an architectural invariant). Transform insert/remove/relocate keep the grid in sync. `World::spatial_candidates` returns leave-rect + replication-class entities with no hysteresis and no `ConnectionId`. Enter/leave rectangles (`AOI_POLICY_HALF_EXTENTS = [16, 9]`, `AOI_LEAVE_MARGIN = 2`) are **server interest policy / development constants**, not client 16:9, camera clamp, or viewport height. Hysteresis lives only in `ObserverReplicationState`. VisibleObservers that participate in AOI must have Transform; missing Transform is not globally visible. OwnerOnly may be non-spatial via the owner path.
- Reason: Full-map broadcast and independent world scans do not scale. Callers must use the World query/candidate APIs. Encoding client aspect into AOI would couple presentation to replication.
- Consequences: `relevance_for` remains a non-hysteretic alias of spatial candidates. Grid internals stay in simulation; the client overlay may draw policy rects for debugging only.
