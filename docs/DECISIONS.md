...
- Consequences: ADR-0050 is unchanged. This is not a gameplay phase; root `PHASE` stays `6G`. Do not start Phase 7 from this decision. Do not add map/NPC/content editors, an admin protocol, or a web/Electron stack here. Probe persistence debt (`dev.probe`) is unchanged. Slice 1.1 adds an application shell (Dashboard / Runtime Server / Logs live; other nav entries are placeholders). Slice 2 adds Runtime Validation orchestration (`purgatory-load` remains pass/fail authority) and a workspace-scoped Hub lock (`logs/dev-tools/hub.lock`); the harness is session-owned and dies with the Hub, the dedicated server does not. Details: [`docs/dev-tools/`](dev-tools/README.md), [`docs/dev-tools/PARITY.md`](dev-tools/PARITY.md).
