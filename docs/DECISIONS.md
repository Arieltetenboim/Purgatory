...
- Leave `Graphic/` exactly where it is. Do not move, rename, modify, import, scan, or connect it to the runtime until later visual-content phases, except: Phase 5.0B Connection Frontend loading **only** `Graphic/LOGO.png` (temporary filesystem path); and the game client compile-embedding the four Animation Lab Headwear Side proof PNGs (`equipment.debug.headwear_proof.a.side` through `d.side`) via `include_bytes!` (no runtime Graphic/ loader; Player debug overlay selects cells 1–4).
...
- Status: accepted
- R5A supersession: ADR-0065 supersedes only the one-login/one-CharacterId mapping and first-login allocation contract below. The distinct CharacterId namespace and per-Character occupancy/session principles remain accepted. The original Phase 6E decision is retained as history.
...
- Status: accepted
- Decision: An `Action` slot exists only after start succeeds. Request / validate / reject are command-pipeline outcomes, not stored `ActionPhase` variants. Stored live phase is `Windup`, `Active`, or `Recovery`; terminal is `Completed`, `Cancelled`, `Interrupted`, or `Failed`. Identity is generational `ActionId`. Foundational policy: at most one **live** action per owner (Windup/Recovery occupy the exclusive slot). `ActionDenialReason` is typed (`MissingOwner`, `Busy`, `TransitionLocked`, `Disconnected`); there is no `can_act: bool`. `InputGateReason` is absorbed as `TransitionLocked` without rewriting ADR-0042. `InteractionSession` remains the 6B interaction session and is not merged into Action. Owner despawn cancels the live action. Illegal transitions and double-complete are rejected. Phase **9A** abilities reuse this table (`ActionKind::Ability`); they do not add a second state machine. 7.2 `ActionKind::Strike` remains load-validation workload, not an authored ability.
- Reason: Persisting every conceptual planning name (`Requested`, `Validated`, `Rejected`) would duplicate the command pipeline inside World. Interaction and exclusive actions have different lifetimes and wire contracts. Ability windup/recovery are live exclusive timing, not command-pipeline names.
- Consequences: Synthetic `ActionKind::Test`, workload `Strike`, and content-driven `Ability`. Existing Interact / Portal / DevSetChannel envelopes stay on their current protocol versions and call the shared gate/preamble internally. Ability definitions and effect execution: ADR-0061.
...
- Consequences: Developer Tools is not a Cargo workspace member. Closing the UI does not stop game processes; reopen adopts workspace `target\` processes and verifies them. `--probe` currently follows the normal DEV persist/enter path (known debt; see `docs/dev-tools/`). Map/NPC editors and Phase 7 capacity/gameplay work are out of scope for this decision. Details: [`docs/dev-tools/`](dev-tools/README.md).

## ADR-0051: Load validation is test infrastructure (Phase 6G)

- Status: accepted
- Decision: Synthetic World population, scheduler/action/effect/event/cadence pressure, and isolated persist for validation live behind one namespaced env: `PURGATORY_LOAD_VALIDATION` (JSON). The server applies it only when load-mode is explicit (`PURGATORY_ADMISSION_CAP` set). Production gameplay paths must not depend on `ScheduledKind::TestProbe`, `ActionKind::Test`, or `EffectKind::Test`. The adopted live UDP `PURGSTAT` contract remains **schema 3**. Schema 4 execution totals were inventoried so Mixed validation could prove work ran when 1 Hz gauges stay zero, then **deferred** (not adopted as the runtime metrics contract). Pass/fail remains `purgatory-load`. Soak duration is configurable; a ~30 minute Mixed run is initial evidence, not the definition of a soak. Developer Tools Runtime Validation forwards `purgatory-load --preset` argv/env; CLI owns pass/fail.
- Reason: Phase 6G is integrated validation of 6A–6F, not a second runtime. Accidental production dependence on test kinds would block later actor/NPC work that must use the generic Entity spine without a fake Character.
- Consequences: Protocol stays v10. No CharacterId on Welcome. Failure injection uses temp/run dirs, never `%LOCALAPPDATA%\Purgatory`. Report: [`docs/PHASE_6G_REPORT.md`](PHASE_6G_REPORT.md). Exit review: [`docs/PHASE_6_EXIT_REVIEW.md`](PHASE_6_EXIT_REVIEW.md). Do not start Phase 7 from this decision. Phase 7 is capacity / production scaling ([`docs/PHASE_7_PLAN.md`](PHASE_7_PLAN.md)), not MOB-as-phase-purpose.
...
- Status: accepted
- Decision: Reinterpret the Phase 6G endpoint as an explicit capacity-engineering track that **closed inside 6G** (6G.1 correctness → 6G.2 characterization → 6G.3–6G.7C remediation/foundation). **6G.1** = correctness / integrated validation (automated GREEN; presentation/duration P0 closed). **6G.2** = capacity characterization: freeze a reference build ([`docs/MMO_RUNTIME_BASELINE.md`](MMO_RUNTIME_BASELINE.md)), coarse per-tick domain timings + process CPU/memory as **run artifacts** (not stuffed onto UDP), five scenario ladders (idle / distributed / mutual-AOI hotspot / churn / gameplay-service burst) at 32→64→128→256, then an instrumented ~30-minute Mixed soak. Live UDP `PURGSTAT` remains **schema 3**; domain timings stay file artifacts. Crossing 33.33 ms tick work is a recorded capacity signal, not an automatic ladder stop. **6G.2 does not redesign** AOI, replication, scheduler, or storage. **6G.3+** = bottleneck remediation only after owner review of measured evidence (measure → review → redesign). **6G.4** performance-regression budgets from Pass 1 were **not** required to close 6G. Root `PHASE` is `6G` now that architecture is closed.
- Reason: Progress means proving the existing runtime under measured MMO-like pressure and redesigning only where evidence justifies it—not treating new gameplay systems as the default form of progress while capacity remains unknown.
- Consequences: Remaining capacity, ownership, and production-scaling work is **Phase 7** ([`docs/PHASE_7_PLAN.md`](PHASE_7_PLAN.md)), not a 6G reopen and not a gameplay-vocabulary phase. Do not raise `PREDICTION_PENDING_CAP` or admission as a “fix.” Localhost numbers are not player-capacity claims. The previously observed 128-client stall is **not** an established server bottleneck (investigated in 6G.2; later 6G.7B/C showed cheap replication at 128). Future major systems must declare an expected runtime cost model. Do not start Phase 7 from this ADR alone.

## ADR-0054: Wire-visible NPC kind (Phase 7.2)

- Status: accepted
- Decision: Protocol **v11** adds `ReplicatedKind::Npc = 4` for authoritative visible non-player actors. Server `snapshot_entity` emits Npc for `EntityKind::Generic` with Transform (not Interactable, not Platform). Existing Enter optional health + Update health domain are reused. NPCs must **not** masquerade as `Player` or `Interactable`. Client draws Npc as a distinct quad; E-interact and Portal routing exclude Npc. Simulation identity remains `EntityKind::Generic` + optional `NpcState` capability.
- Reason: Phase 6 AOI already classifies visible Generics, but they never Entered the wire. Capacity realism for 7.2 requires observer Enter/Update/Leave for NPC workload without reopening AOI/fan-out architecture.
- Consequences: Incompatible Hello/Welcome version bump (v10 peers rejected). Historical v1–v10 goldens stay frozen. 6F DEV probe and load `synthetic_entities` Generics become wire-visible as Npc. No player-issued `ActionRequest` control; combat remains a workload skeleton only. N10b later narrows the original E-interact exclusion: an equipment-capable humanoid Social NPC remains `ReplicatedKind::Npc` and may be an advisory E target, while the server's `Interactable` capability remains authoritative. Report: [`docs/PHASE_72_REPORT.md`](PHASE_72_REPORT.md).

## ADR-0056: Single-process authoritative World remains canonical until measured triggers (Phase 7.7)

- Status: accepted
- Decision: The canonical scaling architecture remains **one dedicated server process → one `GameplayOwner` → one authoritative `World` tick owner**. Many `WorldAddress` values (map × channel × instance) may exist inside that World. Phase 7.3–7.6 evidence does **not** justify implementing intra-tick simulation parallelism, parallel replication, zone partitioning, multi-process worlds, or cross-process handoff. Prefer the evolutionary path: local owner optimization → optional staged replication parallelism → independent channels/instances → multi-process independent worlds → zone/live handoff only if required. Reopen implementation only under the measurable trigger contract in [`docs/PHASE_77_REPORT.md`](PHASE_77_REPORT.md) (§7.7B) or an explicit product/world requirement kept separate from performance saturation.
- Reason: Measured envelope stays comfortably under the 30 Hz tick budget (util ≲11% through mixed@128 / dense@64). No CPU/transport/memory/simulation saturation was demonstrated. Parallelism is not justified. `replication_policy` is only the **projected** first limiter. Shipping distributed complexity without a measured limit would raise correctness and operational risk for no capacity gain.
- Consequences: Phase 7.7 introduces **no** scaling implementation. Production performance expectations belong in Phase 7.8 and living budgets — not invented player caps here. Channels/instances remain the preferred **first horizontal unit** when isolation is needed, aligned with existing `WorldAddress` contracts, but are not authorized as a speculative build. Report: [`docs/PHASE_77_REPORT.md`](PHASE_77_REPORT.md).

## ADR-0057: Protocol v12 equipment authority and replication (Phase 8C)

- Status: accepted
- Decision: Bump `PROTOCOL_VERSION` from 11 to 12. Add control envelopes `Equip` (tag 18) / `Unequip` (tag 19) and `Equipment` Accepted/Rejected (tags 20/21). Add equipment as replication domain bit 2 on Enter (full `Option<ReplicatedEquipment>`) and Update (slot delta). Per-connection equipment sequence reuses `SessionInput::classify` independently of movement input. Server authorizes from gameplay `EquipmentDefinition` only. Observer `CommittedRevs` tracks equipment revision and last sent `Option<EquipmentState>` so deltas are per-observer, not globally consumed.
- Reason: v11 had no versioned frame decoder. Bolting equipment onto v11 Enter/Update would be an incompatible silent layout change. Equipment is appearance for all relevant observers; it must not be dropped like stranger health. Persistent truth is replicated state, not Accepted events.
- Consequences: v11 peers are rejected at Hello. Historical v1–v11 goldens stay frozen. Presentation/skeleton/ART remain off the wire. Report: [`docs/PHASE_8C_REPORT.md`](PHASE_8C_REPORT.md).

## ADR-0058: Standalone authoring labs launched from Developer Hub

- Status: accepted
- Decision: Presentation authoring tools (starting with **A7.0 Animation Lab**) are **separate eframe desktop binaries**, not the game client and not pages inside the fixed-size Developer Hub window. The Hub provides a launch action (Content page + Dashboard Quick Action) and spawns the lab as a detached process (`spawn_visible`, breakaway from the Hub job). Lab lifetime is independent of Hub close, Kill All, and any running game client. The lab may depend on `purgatory-animation` and `purgatory-skeleton` plus eframe/egui. It must not depend on client, server, simulation, protocol, content, common, Quinn, Tokio, or wgpu. A6 `.anim` parse/serialize/validation in `purgatory-animation` remains the asset contract. Runtime clips stay compile-time `include_str!`; observing a saved clip in-game requires a client rebuild. Command/history Undo/Redo is part of A7.0. Notify dispatch, curves, ART, IK, and A7.1 QoL are not authorized here.
- Reason: Embedding an animation editor in the game client would couple authoring to a live session. Embedding it in the Hub would fight the Hub’s fixed 1280×800 runtime-control layout. A launched binary matches existing Hub tool-launch (quality gate, clients) and keeps crate boundaries intact.
- Consequences: ADR-0052’s “no content editors in that Hub slice” is unchanged for map/NPC editors. Content is now a live Hub page for **launching** Animation Lab, not for hosting the editor UI. World remains a placeholder. Kill All does not terminate authoring labs.
...
## ADR-0060: Client `dev-diagnostics` Cargo feature (D4)

- Status: accepted
- Decision: `purgatory-client` defaults to feature **`dev-diagnostics`**, which pulls optional `egui` / `egui-winit` / `egui-wgpu` and compiles the in-window Debug overlay, Connection Frontend, `DebugCommand`, `DiagnosticsFrame` assembly, impairment UI, jitter isolation, and privileged proof surfaces. Shipping-style builds use `--no-default-features` (or omit the feature). Without the feature: no egui link, no overlay/commands, time scale fixed at 1.0, network impairment forced Off at connect (env cannot enable it), presentation proof/force flags compile to off, and the client **auto-connects once** on the Connection screen (no Connection Frontend). Product character draw (`skeleton_debug` placeholders / presentation quads) remains. This amends ADR-0016’s “later release profile may compile the overlay out” note. Server DEV-handler strip is **not** part of D4 (explicit follow-up).
- Reason: Privileged diagnostics must be absent from shipping binaries by compile-time gating, not by hiding widgets. Capacity/DEV `cargo run --release` still needs the overlay, so the gate is a Cargo feature rather than `cfg(debug_assertions)`.
- Consequences: Normal developer and Hub client builds keep default features. Shipping proof: `cargo check/clippy/test -p purgatory-client --no-default-features`. D5 IPC hooks remain reserved behind the same feature surface. No protocol change.

## ADR-0061: Ability foundation reuses Action Runtime (Phase 9A)

- Status: accepted
- Decision: Gameplay abilities use the existing Action Runtime as lifecycle owner. `AbilityDefinition` is the minimum content-driven shape (`AbilityId` = `ContentId`, timing, `AbilityActivation`, `AbilityDelivery`, ordered `effects[]`). **Activation ≠ affected entities.** Independent activation does not require a selected target. Instant Health mutation goes through `AbilityEffect` (`Damage` in v1) via `World::execute_ability_effect` → `apply_damage`. Ability-specific code must not `set_health`. `TempEffect` remains duration-based and is not the ability effect list. Health stays an optional World capability; Alive/Dead is derived from `Health.current`. Cooldown is a World table keyed by `(EntityId, AbilityId)`, not a default combat component. The client will request an ability (id + optional selected entity), never authoritative damage or hit results. Health remains the replicated combat-vitality domain; live action phase and cooldowns are server-only until a later slice adds a tag. Presentation consumes semantic Attack/Hurt (existing Phase 8 oneshots); Dead is Health-derived. 7.2 `Strike` is unchanged workload, not Basic Attack. Protocol **v14** is unchanged in 9A/9B. Phase **9B** authors `skill.basic.strike` as shared JSON and executes a server forward AABB query on Active.
- Reason: A parallel Ability state machine would duplicate ADR-0046 gates, exclusive-owner policy, and the scheduler. A GAS-like framework is out of scope. World cannot depend on `purgatory-content`, so the runtime definition type lives in simulation; JSON packing is in `purgatory-content`. Requiring a selected target to activate Basic Attack was incorrect.
- Consequences: Players/NPCs/bosses/PvP later share `World::request_ability`. Encounter scripts may call it without forcing minigames through Action. Phase **9C** added the client command and protocol **v15**. Reports: [`docs/PHASE_9A_REPORT.md`](PHASE_9A_REPORT.md), [`docs/PHASE_9B_REPORT.md`](PHASE_9B_REPORT.md), [`docs/PHASE_9C_REPORT.md`](PHASE_9C_REPORT.md).

## ADR-0062: AbilityActivate protocol v15 and grant table (Phase 9C)

- Status: accepted
- Decision: Bump `PROTOCOL_VERSION` from 14 to 15. Add client `AbilityActivate` (tag **25**: seq + ability id + optional selected entity) and server `Ability` Accepted/Rejected (tags **26**/**27**). Do not reuse `InputCommand` or `Equip` for activation. `seq` follows the equipment duplicate/stale/gap rules. `World` owns an explicit `AbilityGrantTable` answering which abilities an entity may activate; live players are granted `skill.basic.strike`. Future grant sources (progression, equipment, status) write the same table. The network handler checks grants and activation-shape; `World::request_ability` remains the generic 9B runtime. Live server player spawn attaches Health; `World::spawn_player` does not. `nearest_health_target` skips players so 7.2 Strike stays workload-only.
- Reason: Ability activation is not movement intent and is not an equipment mutation. Distortion of those envelopes would hide an incompatible command. Grants must be explicit before `request_ability` so unknown/ungranted ids cannot execute effects.
- Consequences: v14 peers are rejected at Hello. Historical goldens stay frozen. Client `J` sends Independent Basic Strike with no target. Phase **9D** wires Attack/Hurt/Dead presentation. Report: [`docs/PHASE_9C_REPORT.md`](PHASE_9C_REPORT.md).

## ADR-0063: Ability presentation cues reuse oneshots; Dead is Health-derived (Phase 9D)

- Status: accepted
- Decision: Ability execution starts semantic Attack via the existing presentation oneshot table and `ServerPresentationOneShot` fanout — independent of hits, delivery shape, and ability id. Authoritative non-lethal `apply_damage` starts Hurt the same way. Lethal damage clears transient oneshots; persistent Dead is `PresentationActivity::Dead` derived from replicated `Health.current <= 0` on both local and remote characters. Character Presentation precedence is Dead > oneshot > locomotion. No protocol bump (v15). Animation Runtime and Skeleton stay unaware of Health/ability rules. Dead uses the Hurt clip as a deterministic placeholder until an authored death clip exists.
- Reason: Phase 9 separated activation from affected targets; Attack must follow execution, not hit confirmation. Dead is state, not a oneshot. Reusing v13 oneshot envelopes avoids inventing clip/bone traffic.
- Consequences: Empty swings show Attack. Observers and attacker share the same semantic events. 7.2 Strike damage also emits Hurt if Health changes (same `apply_damage` boundary). Report: [`docs/PHASE_9D_REPORT.md`](PHASE_9D_REPORT.md).

## ADR-0064: Production UI text through Glyphon and cosmic-text

- Status: accepted
- Decision: The client uses Glyphon 0.12.0 and its matching cosmic-text 0.19.0 types behind TextBlock. PURGATORY owns bundled fonts, logical typography units, linear color semantics and ordered UI composition; cosmic-text owns shaping/layout/font matching; Glyphon owns GPU glyph residency and rendering. Typography converts once through UI Scale and window scale to physical metrics. Distinct prepared composition batches use persistent renderer slots sharing an atlas.
- Reason: Upgrading the ab_glyph path would leave shaping, bidi, fallback and atlas policy as custom integration work. cosmic-text with a custom GPU atlas would still duplicate GPU ownership already provided by a released Glyphon compatible with wgpu 30.0.1. Neither wgpu nor winit needs an upgrade.
- Consequences: Text v0's fixed-size raster and character layout are retired. Text remains client presentation and keeps panel/text ordering. Phase 2A uses the existing bundled DejaVu Sans Bold face; final fallback policy and complete face variants remain 2B. See TEXT_RENDERING.md for the implemented contract and validation limits.


## ADR-0064: Dash is an input-anchored authoritative ability (Protocol v31)

- Status: accepted
- Decision: Dash reuses `AbilityDefinition`, the Action Runtime, cooldowns and explicit grants. Protocol v31 extends `AbilityActivate` with `(input_epoch, input_sequence)` instead of folding ability activation into `InputCommand`. A valid activation waits for that authoritative input boundary; it adopts same-step horizontal intent, then executes before `tick_player`. The client predicts at the same boundary and restores/replays from recipient-local replicated Dash state. For a locally predicted input-anchored ability, the client enqueues the ability request and its owning `InputCommand` as one transport-queue item and writes `AbilityActivate` before the input anchor on the reliable control stream. Dash v1 is ground-only, locks facing, ignores steering/jump, locks other ability activation for its active+recovery window, resolves through FOOTNOTE collision, stops on walls/edges, uses an authored 60-tick cooldown mirrored by client prediction, and actual damage interrupts both its Action and velocity. Repeated Dash/Attack edges during the Dash action window are consumed locally without producing new ability requests; the server independently rejects attempts while Dash is pending or active. Dash is content-loaded but not granted at spawn; the Welcome gate watchman demonstrates learned progression through `GrantAbility`. Learned-grant persistence remains deferred.
- Reason: Direction-sensitive movement must share a deterministic tick boundary across server and prediction, while ability authorization/lifecycle stays separate from held movement intent.
- Consequences: v30 peers are rejected. Server control adds owner-private `AbilityGrants` tag 44. `ReplicationFrame` gains recipient-local optional Dash state. Transition, respawn and HeldCancel barriers reject pending activations. Crouch and broader VFX systems are not introduced by this decision.

## ADR-0065: Authoritative DEV character rosters (R5A)

- Status: accepted
- Decision: `CharacterId` remains a server-minted persistent character identity, distinct from runtime identity. `DevLogin` identifies a temporary DEV profile/roster with zero to three Characters in persistent slot order; it is not a production account. The existing single mutable persistence owner allocates IDs, owns roster membership and enforces globally unique character names. Shared `CharacterName` accepts 3–12 ASCII alphanumeric characters, preserves display case, and supplies an ASCII-case-insensitive uniqueness key. Client presentation/error wording remains client-owned.
- Storage: `identity.json` schema v2 stores `schema_version`, `next_character_id`, and login → ordered `{ character_id, display_name }` entries. Creation validates the name, capacity and global uniqueness, then writes a candidate state through the existing recoverable tmp/bak replacement path before publishing it in memory. Failed persistence leaves the roster, allocator and name ownership unchanged. Corrupt ownership, names, capacity or allocator state is rejected without repair. `next_character_id` must exceed all owned IDs, except that `u64::MAX` with MAX already owned marks exhaustion and cannot allocate again.
- Migration: Schema v1 mappings are sorted by CharacterId, then login, and assigned deterministic base-36 ordinal compatibility names (`000` through `zzz`, then `0000` through `zzzz`, continuing through length 12). These names have no gameplay meaning and never determine an ID. Every existing CharacterId is preserved exactly, including large u64 values. Only the actual finite dataset needs names; namespace exhaustion is an explicit migration error. The validated result is persisted as v2 through the same recoverable replacement path. Future schema versions are rejected. Existing `char_{id:016x}.json` gameplay files and their schema remain unchanged.
- Reason: Phase 6E's one-login/one-character mapping cannot represent the R4 three-slot frontend contract. Roster summaries belong to the identity owner rather than gameplay snapshots or a parallel account file.
- Consequences: `PersistenceService::roster` returns owned summaries without filesystem scans; `create_character` returns the authoritative entry; `owns_character` checks membership. New entries need no gameplay file until `load_or_default` materializes one. Temporary direct-play `resolve_or_create` loads slot zero, or creates one deterministic unused compatibility name through the same creation operation when empty. No networking, occupancy, simulation ownership or gameplay record changes are introduced. Real accounts/authentication, selected-character networking, deletion and rename remain deferred. File persistence continues to require one writer; this decision does not add multi-process locking or stronger atomic-replace guarantees.


## ADR-0066: Connected session is distinct from gameplay entry (R5B)

- Status: accepted
- Decision: Keep ClientScreen Connection/Game and ConnectionState unchanged. Connected describes an accepted transport/session; FrontendSessionReady establishes a pre-game session with authoritative ordered roster metadata. Only explicit authoritative gameplay readiness (Welcome) can authorize Game. R5B deliberately has no production gameplay entry path.
- Ownership: A live connection retains its validated DevLogin. Roster/create use the existing control connection and single mutable persistence worker. The client projects server IDs/names into three slots and sends only a name for creation. Pending creation locks the workspace; normal domain rejection preserves the draft. Disconnect clears the projection, and lifecycle attempt filtering precedes projection.
- Reason: Phase 6E's Hello → resolve/create → Enter → Welcome conflated login, character identity and gameplay admission, preventing empty rosters and server-backed creation before play. ADR-0043 occupancy remains character-entry scoped; ADR-0065 persistence authority remains unchanged.
- Consequences: No occupancy reservation, replication stream or player spawn on login. No second runtime, reconnect between frontend stages, or production channel command. Historical gameplay fixtures retain explicitly requested test-only legacy setup; production DEV controls use the new session. Readiness probes check session readiness. Gameplay bots await the next selected-character entry slice. DevLogin remains a DEV profile key, not authentication.
