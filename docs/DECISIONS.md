...
- Status: accepted
- Decision: Dash reuses `AbilityDefinition`, the Action Runtime, cooldowns and explicit grants. Protocol v31 extends `AbilityActivate` with `(input_epoch, input_sequence)` instead of folding ability activation into `InputCommand`. A valid activation waits for that authoritative input boundary; it adopts same-step horizontal intent, then executes before `tick_player`. The client predicts at the same boundary and restores/replays from recipient-local replicated Dash state. During late-collapse, all queued ability anchors inside the collapsed prefix execute once on that authoritative step in accepted request order. Dash v1 is ground-only, locks facing, ignores steering/jump, resolves through FOOTNOTE collision, stops on walls/edges, and actual damage interrupts both its Action and velocity. Dash is content-loaded but not granted at spawn. Learned progression is a distinct grant source and the future NPC seam; learned-grant persistence remains deferred.
- Reason: Direction-sensitive movement must share a deterministic tick boundary across server and prediction, while ability authorization/lifecycle must stay separate from held movement intent (ADR-0062). Replicating the small durable Dash state makes correction possible without trusting client-authored speed, direction or duration.
- Consequences: v30 peers are rejected. Server control adds owner-private `AbilityGrants` tag 44. `ReplicationFrame` gains recipient-local optional Dash state. Transition, respawn and HeldCancel barriers reject pending activations. Crouch and VFX systems are not introduced by this decision.
