...
S2 evaluates **one** Humanoid v0 on the **local presented pose** (same `frame_local.presented` used by the player AABB). The adapter maps AABB center → skeleton root near the body feet (`center.y - PLAYER_HALF_EXTENTS[1] + CHARACTER_COLLISION_FOOT_OVERLAP`). The positive overlap (currently `0.1762` world units) is presentation-only and is calibrated so the current base character sole aligns with the player AABB bottom at the normal 1.15 presentation scale; skeleton rest proportions stay independent of collision.
