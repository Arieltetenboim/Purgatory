...
def next_monster_content_id(repo_root: Path) -> int:
    catalog = load_numeric_catalog(repo_root)
    used = {
        raw
...
def prepare_monster_allocation(
    ...
    if re.search(rf"\b{re.escape(constant)}\b", catalog):
        raise ValueError(f"Catalog constant {constant} already exists for another allocation.")
    ...
    pos = constant_matches[-1].end()
    catalog = (
        catalog[:pos]
        + f"\npub const {constant}: ContentId = ContentId::from_raw({content_id // 1000}_{content_id % 1000:03d});"
        + catalog[pos:]
    )
...
,
    ...
    pos = label_matches[-1].end()
    indent = re.match(r"^\s*", label_matches[-1].group(0)).group(0)
...
,
    ...
    pos = reverse_matches[-1].end()
    indent = re.match(r"^\s*", reverse_matches[-1].group(0)).group(0)
    catalog = catalog[:pos] + f'\n{indent}{constant} => "{authored_id}",' + catalog[pos:]

    monster_section = re.search(
        r"(### Monsters — 10,000–19,999\s*\n\s*\| ID \| Label \| Status \|\s*\n\s*\|[^\n]+\|\s*\n)(.*?)(?=\n### |\Z)",
        ledger,
        flags=re.DOTALL,
    )
