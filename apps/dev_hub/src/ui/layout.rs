...
impl PageOutcome {
    pub fn none() -> Self {
        Self { command: None, navigate: None }
    }

    pub fn command(cmd: purgatory_dev_runtime::HubCommand) -> Self {
        Self { command: Some(cmd), navigate: None }
    }
...
pub fn page_header(ui: &mut egui::Ui, title: &str, blurb: &str) {
    ui.label(RichText::new(title).font(theme::title_font()).strong().color(theme::body()));
    if !blurb.is_empty() {
        ui.add_space(2.0);
        ui.label(RichText::new(blurb).font(theme::subtitle_font()).color(theme::muted()));
    }
...
pub fn chip(ui: &mut egui::Ui, text: &str) {
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            ui.label(RichText::new(text).font(theme::subtitle_font()).color(theme::muted()));
        });
}
...
pub fn status_badge(ui: &mut egui::Ui, label: &str, color: Color32) {
    ui.horizontal(|ui| {
        ...
        ui.label(RichText::new(label).color(color).strong().font(theme::section_font()));
    });
}
...
pub fn metric_row(ui: &mut egui::Ui, key: &str, value: &str, mono: bool) {
    ui.horizontal(|ui| {
        ui.spacing_mut().item_spacing.x = 6.0;
        ui.label(RichText::new(key).font(theme::subtitle_font()).color(theme::muted()));
...
pub fn module_title(ui: &mut egui::Ui, icon: &str, title: &str) {
    ui.horizontal(|ui| {
        ...
        ui.label(RichText::new(title).font(theme::section_font()).color(theme::muted()).strong());
    });
    ui.add_space(2.0);
}

pub fn btn_primary(label: &str) -> egui::Button<'static> {
    egui::Button::new(RichText::new(label.to_owned()).strong().color(Color32::WHITE))
        .fill(theme::accent())
        .stroke(Stroke::NONE)
...
pub fn btn_destructive(label: &str) -> egui::Button<'static> {
    egui::Button::new(RichText::new(label.to_owned()).strong().color(theme::destructive()))
...
pub fn nav_item(ui: &mut egui::Ui, selected: bool, icon: &str, label: &str) -> egui::Response {
    ...
    if selected {
        ...
        let bar = egui::Rect::from_min_size(egui::pos2(rect.left(), rect.top() + 4.0), Vec2::new(3.0, h - 8.0));
        painter.rect_filled(bar, 1.5, theme::accent());
    } else if resp.hovered() {
        painter.rect_filled(rect, 6.0, Color32::from_rgb(24, 30, 38));
    }
    let text_color = if selected { theme::body() } else { theme::muted() };
    let mut x = rect.left() + 10.0;
    if !icon.is_empty() {
        painter.text(egui::pos2(x, rect.center().y), egui::Align2::LEFT_CENTER, icon, theme::subtitle_font(), text_color);
        x += 18.0;
    }
    painter.text(
        ...
        label,
        theme::section_font(),
        if selected { theme::body() } else { Color32::from_rgb(170, 176, 186) },
    );
    resp
}
...
pub fn log_scroll_view(
    ...
    egui::ScrollArea::both()
        ...
        .show(ui, |ui| {
            ...
            for line in lines {
                ui.add(
                    egui::Label::new(RichText::new(line).font(theme::mono_small()).color(theme::log_color(line)))
...
pub fn log_panel(
    ...
    ui.horizontal(|ui| {
        ...
        if !hint.is_empty() {
            ui.colored_label(theme::muted(), RichText::new(hint).font(theme::subtitle_font()));
        }
        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            if ui.add(btn_ghost("Clear")).clicked() { clear = true; }
        });
    });
    ui.add_space(2.0);
...
pub fn bounded_log_panel(ui: &mut egui::Ui, id_salt: &str, lines: &[String], empty: &str) -> bool {
    log_panel(ui, id_salt, lines, empty, theme::LOG_VIEW_HEIGHT, "scroll ↕↔ for long lines")
}
...
pub enum ChartSeries { Connected, TickMean }
...
pub fn metrics_chart(
    ...
    ui.horizontal(|ui| {
        ...
        egui::ComboBox::from_id_salt(format!("{id_salt}_series"))
            ...
            .show_ui(ui, |ui| {
                ui.selectable_value(selected, ChartSeries::Connected, ChartSeries::Connected.label());
                ui.selectable_value(selected, ChartSeries::TickMean, ChartSeries::TickMean.label());
            });
    });
    if !series.available || series.samples.is_empty() {
        ui.colored_label(theme::muted(), "No metrics.csv samples yet (harness writes ~1 Hz).");
        return;
    }
    let samples = &series.samples;
    let values: Vec<(f64, f64)> = samples.iter().filter_map(|s| {
    ...
    if values.is_empty() {
        ui.colored_label(theme::muted(), "Selected series has no numeric samples yet.");
        return;
    }
    let min_v = values.iter().map(|(_, v)| *v).fold(f64::INFINITY, f64::min);
    let max_v = values.iter().map(|(_, v)| *v).fold(f64::NEG_INFINITY, f64::max);
    let pad_v = ((max_v - min_v).abs() * 0.08).max(if matches!(*selected, ChartSeries::Connected) { 0.5 } else { 0.05 });
    ...
    ui.horizontal(|ui| {
        ...
        ui.separator();
        ui.colored_label(theme::muted(), format!("min {}   max {}   x: {} → {}", format_series_value(*selected, min_v), format_series_value(*selected, max_v), format_secs(t0), format_secs(t1)));
    });
    ...
    let top_pad = 8.0;
    let right_pad = 10.0;
    let plot = egui::Rect::from_min_max(egui::pos2(rect.left() + left_axis, rect.top() + top_pad), egui::pos2(rect.right() - right_pad, rect.bottom() - bottom_axis));
    ...
    for i in 0..=4 {
        ...
        painter.line_segment([egui::pos2(plot.left(), y), egui::pos2(plot.right(), y)], Stroke::new(1.0, grid));
        let v = axis_min + f64::from(frac) * span_v;
        painter.text(egui::pos2(plot.left() - 4.0, y), egui::Align2::RIGHT_CENTER, format_axis_value(*selected, v), font.clone(), axis);
    }
    for i in 0..=4 {
        let frac = i as f32 / 4.0;
        let x = plot.left() + frac * plot.width();
        painter.line_segment([egui::pos2(x, plot.top()), egui::pos2(x, plot.bottom())], Stroke::new(1.0, grid));
        let t = t0 + f64::from(frac) * span_t;
        painter.text(egui::pos2(x, plot.bottom() + 4.0), egui::Align2::CENTER_TOP, format_secs(t), font.clone(), axis);
    }
    painter.line_segment([egui::pos2(plot.left(), plot.top()), egui::pos2(plot.left(), plot.bottom())], Stroke::new(1.0, axis));
    painter.line_segment([egui::pos2(plot.left(), plot.bottom()), egui::pos2(plot.right(), plot.bottom())], Stroke::new(1.0, axis));
    let mut points = Vec::with_capacity(values.len());
    for (t, v) in &values {
        let tx = ((*t - t0) / span_t) as f32;
        let vy = ((*v - axis_min) / span_v) as f32;
        points.push(egui::pos2(plot.left() + tx * plot.width(), plot.bottom() - vy * plot.height()));
    }
    if points.len() >= 2 {
