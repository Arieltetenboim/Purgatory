...
pub enum Alignment {
    ...
    Left,
    Center,
    #[cfg_attr(not(test), allow(dead_code))] // Text v0 API; the single proof uses Center.
    Right,
}
...
impl TextLayout {
    fn new(
        ...
        if !style.valid() || viewport.contains(&0) || !anchor.iter().all(|v| v.is_finite()) {
        ...
        for (line_number, line) in content.0.split('\n').take(MAX_GLYPHS).enumerate() {
            if remaining == 0 {
                break;
            }
