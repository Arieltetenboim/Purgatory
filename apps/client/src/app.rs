...
impl ClientApp {
    ...
    fn send_ability_for_tick(&mut self, command: InputCommand) {
        ...
        if !self.lifecycle.gameplay_actions_allowed()
            || self.gameplay_input_locked()
            || self.dialogue_runtime.is_active()
        {
            let _ = self
                .network
                .as_ref()
        ...
        let client_tick = self.clock.tick().get();
        // Dash owns the whole local ability window, not only the movement
        // frames. Consume edges, but do not send/predict any Ability while the
        // authored Dash active+recovery window is still live.
        if self.prediction.ability_activation_locked(client_tick)
            || self
                .world
                .player_id()
                .is_some_and(|actor| self.world.player_dash_of(actor).is_some())
        {
            let _ = self
                .network
                .as_ref()
            ...
            let dash = definition.effects.iter().find_map(|effect| {
                ...
                } = *effect
                {
                    Some((
                        speed,
                        duration_ticks,
                        definition.timing.cooldown_ticks,
                        definition
                            .timing
                            .windup_ticks
                            .saturating_add(definition.timing.active_ticks)
                            .saturating_add(definition.timing.recovery_ticks),
                    ))
                } else {
                    None
                }
            });
            if dash.is_some() && !self.prediction.dash_prediction_ready(client_tick) {
                continue;
            }
        ...
        // Predict Dash before enqueueing so a local precondition failure does
        // not create a request that can only come back as a corrective reject.
        if authored == "skill.movement.dash"
            && let Some((speed, duration_ticks, cooldown_ticks, activation_lock_ticks)) = dash
            && !self.prediction.try_predict_dash(
                &mut self.world,
                seq,
                command,
                speed,
        ...
        if !self
            ...
            .as_ref()
            .expect("checked")
            .try_send_input_with_ability(command, request)
        {
            eprintln!("9C_ABILITY paired input send failed seq={seq}");
            if authored == "skill.movement.dash" {
                let _ = self.prediction.reject_predicted_ability(
                    seq,
                    &self.replica,
                    &mut self.world,
                    client_tick,
